...
def read_code_file(file_path):
    ...
    with open(file_path) as fp:
        for ln, line in enumerate(fp):
            ...
            line = line.strip()
            if '//' in line:
                line = line[:line.index('//')] # remove comment lines from the original code
...
def extract_nodes_with_location_info(nodes):
    ...
    for node_index, node in nodes.iterrows():
        if math.isnan(node['LINE_NUMBER:int']):
            continue
        else:
            line_num = node['LINE_NUMBER:int']
            node_id = node[':ID']
...
if __name__ == '__main__':
    ...
    nodes_path = "edges_and_nodes/nodes.csv" #os.path.join('parsed', directory, file_name, 'nodes.csv')
    edges_path = "edges_and_nodes/edges.csv" #os.path.join('parsed', directory, file_name, 'edges.csv')
    nodes = pd.read_csv(nodes_path)
    edges = pd.read_csv(edges_path)
    code = read_code_file(code_file_path) # return a dictionary ln ---> code line
